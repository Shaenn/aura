...
const GITHUB_TEMPLATE =
  /^\.github\/(PULL_REQUEST_TEMPLATE\.md|(PULL_REQUEST_TEMPLATE|ISSUE_TEMPLATE)\/[^/]+\.(md|ya?ml))$/i;
...
async function claudeRootOf(slug: string): Promise<string> {
  ...
  if (!existsSync(claudeRoot))
    throw new ProjectError('Ce projet n’a pas de dossier .claude.', 'no-source');
  return claudeRoot;
}
...
async function walkSourceTree(
  root: string,
): Promise<{ memories: ResourceNode[]; repoDocs: ResourceNode[] }> {
  ...
  const rank = (r: ResourceNode): number =>
    r.rel.includes('/') ? 2 : /^README\b/i.test(r.name) ? 0 : 1;
...
export async function listProjectSessions(slug: string): Promise<TranscriptSummary[]> {
  if (!isSlugSafe(slug)) throw new ProjectError('Slug de projet invalide.', 'escape');
  const [transcripts, metrics] = await Promise.all([
 ...
 */
export async function readProjectIncludedFile(
  slug: string,
  rel: string,
...
export async function readProjectMemory(
  ...
  const cleaned = normalize(rel).replace(/^([/\\])+/, '');
  if (!sourceFileKind(cleaned.split(sep).join('/')))
    throw new ProjectError(t('errors.accessDenied'), 'denied');
  const abs = canonicalPath(join(root, cleaned));
  if (!abs.toLowerCase().startsWith((root + sep).toLowerCase()))
    throw new ProjectError('Chemin hors du projet.', 'escape');
  const content = await readFile(abs, 'utf8');
  return { rel: cleaned.split(sep).join('/'), content };
}
...
export async function readProjectTranscriptImage(
  ...
  if (agentId && !ID.test(agentId))
    throw new ProjectError('Identifiant d’agent invalide.', 'escape');
  if (!Number.isInteger(index) || index < 0) {
    throw new ProjectError('Rang d’image invalide.', 'escape');
  }

  const abs = agentId
    ? join(PROJECTS_DIR, slug, safeId, 'subagents', `agent-${agentId}.jsonl`)
    : join(PROJECTS_DIR, slug, `${safeId}.jsonl`);
  if (!existsSync(abs)) throw new ProjectError('Transcript introuvable.', 'not-found');
  ...
  eachImage(
    (found.message as Record<string, unknown> | undefined)?.content,
    (source, _tool, at) => {
