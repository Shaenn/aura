...
export function clavier(f: Formulaire): InlineKeyboardMarkup {
  ...
  const solo: Bouton[] = question.multiSelect
    ? [{ texte: t('passerelle.questionValider'), donnee: `q:${f.id}:ok` }]
    : [];
...
export function ecran(f: Formulaire): {
  ...
  const entete =
    total > 1
      ? t('passerelle.questionEtape', { header: question.header, n: f.etape + 1, total })
      : question.header;
  ...
  question.options.forEach((o, i) => {
    ...
    blocs.push({
      type: 'paragraph',
      text: o.description
        ? [{ type: 'bold', text: titre }, ` — ${o.description}`]
        : { type: 'bold', text: titre },
    });
