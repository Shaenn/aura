...
function bodyFor(rule: RuleName, findings: Finding[], signals: SessionSignal[]): string {
  ...
  const sessions =
    n > 1
      ? t('diagnostics.recommendations.manySessions', { n })
      : t('diagnostics.recommendations.oneSession');
  ...
  switch (rule) {
    ...
    case 'outils-gourmands': {
      ...
      for (const s of signals)
        for (const t of s.tools) tools.set(t.name, (tools.get(t.name) ?? 0) + t.tokens);
    ...
    case 'compaction-lourde': {
      ...
      return t('diagnostics.recommendations.bodies.compaction-lourde', {
        ...
        auto: auto
          ? t('diagnostics.recommendations.bodies.compactionAuto', { n: auto })
          : t('diagnostics.recommendations.bodies.compactionManual'),
      });
    }
    case 'contexte-injecte': {
      const labels = new Map<string, number>();
      for (const s of signals)
        for (const i of s.topInjections) labels.set(i.label, (labels.get(i.label) ?? 0) + i.tokens);
    ...
    case 'fenetre-proche-limite': {
      ...
      return t('diagnostics.recommendations.bodies.fenetre-proche-limite', {
        ...
        auto: auto
          ? t('diagnostics.recommendations.bodies.windowAuto', { n: auto })
          : t('diagnostics.recommendations.bodies.windowNoAuto'),
      });
    }
    case 'rythme-5h':
...
function applyCrossings(recommendations: Recommendation[], findings: Finding[]): void {
  const targetsOf = (rule: RuleName): Set<string> =>
    new Set(findings.filter((f) => f.rule === rule && f.scope === 'session').map((f) => f.target));
  const find = (rule: RuleName): Recommendation | undefined =>
    recommendations.find((r) => r.rule === rule);
...
function buildCaveats(
  ...
  const unpriced = [...new Set(signals.flatMap((s) => s.unpricedModels))]
  ...
  if (uncalibrated.length) {
    caveats.push(
      t('diagnostics.recommendations.caveats.uncalibrated', { rules: uncalibrated.join(', ') }),
    );
  }
...
function problemOf(reco: Recommendation): string {
  const where =
    reco.affected > 1
      ? t('diagnostics.recommendations.manySessions', { n: reco.affected })
      : t('diagnostics.recommendations.oneSession');
...
function toTarget(f: Finding): Target {
  return {
    ...
    label:
      f.scope === 'session'
        ? `${f.target.slice(0, 8)}…`
        : f.target || t('diagnostics.recommendations.wholeFleet'),
...
export function buildReport(
  ...
  for (const [rule, group] of groups) {
    ...
    recommendations.push({
      id: makeId(rule),
      rule,
      severity: group.reduce<Severity>(
  ...
  recommendations.sort(
    (a, b) => weight(b.impact) - weight(a.impact) || a.rule.localeCompare(b.rule),
  );
