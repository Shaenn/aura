...
export function registerProjects(app: FastifyInstance): void {
  ...
  app.get('/api/projects/:slug/transcript/image', async (req, reply) => {
    ...
    if (!id || !uuid) {
      return reply.code(400).send({ error: t('errors.paramsRequired', { first: 'id', second: 'uuid' }) })
    }
    try {
      const { body, mediaType } = await readProjectTranscriptImage(slug, id, uuid, Number(str(query.index, '0')), str(query.agentId))
  ...
  app.get('/api/projects/:slug/tool-result', async (req, reply) => {
    ...
    if (!id || !toolUseId) {
      return reply.code(400).send({ error: t('errors.paramsRequired', { first: 'id', second: 'toolUseId' }) })
    }
    try {
