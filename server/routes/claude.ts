...
export function registerClaude(app: FastifyInstance): void {
  ...
  app.post('/api/claude/propose', async (req, reply) => {
    ...
    if (typeof body.path !== 'string' || typeof body.content !== 'string') {
      return reply
        .code(400)
        .send({ error: t('errors.bodyExpected', { shape: '{ path, content }' }) });
    }
    try {
  ...
  app.post('/api/claude/apply', async (req, reply) => {
    ...
    if (typeof body.path !== 'string' || typeof body.content !== 'string') {
      return reply
        .code(400)
        .send({ error: t('errors.bodyExpected', { shape: '{ path, content, expectedBefore }' }) });
    }
    try {
  ...
  app.post('/api/claude/memory-index', async (req, reply) => {
    ...
    if (
      typeof b.memRel !== 'string' ||
      typeof b.file !== 'string' ||
      (b.action !== 'add' && b.action !== 'remove')
    ) {
  ...
  app.get('/api/claude/memories', async () => {
    ...
    if (await exists('projects')) {
      for (const p of (await listDir('projects')).filter((e) => e.kind === 'dir')) {
        ...
        const files = (await listDir(memRel)).filter(
  ...
  app.get('/api/claude/plugins', async () => {
    ...
    if (rawMk) {
      try {
        const parsed = JSON.parse(rawMk) as Record<
          string,
  ...
  app.get('/api/claude/overview', async () => {
    ...
    return {
      ...
      skills: (await exists('skills'))
        ? (await listDir('skills')).filter((e) => e.kind === 'dir').length
        : 0,
