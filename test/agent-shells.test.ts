...
describe('ShellTracker', () => {
  ...
  it("distingue l'arrêt de la fin, sur le seul `status`", () => {
    ...
    const coupe = FIN.replace('<status>completed</status>', '<status>killed</status>').replace('exit code 0', 'exit code 143')
  ...
  it('garde une fin en échec du côté des fins', () => {
    ...
    const rate = FIN.replace('<status>completed</status>', '<status>failed</status>').replace('exit code 0', 'exit code 1')
  ...
  it('laisse dehors un appel qui n’a jamais tourné', () => {
    ...
    expect(suiveur.consume(repond("Permission to use Bash with command … hasn't been granted"))).toBe(false)
    expect(suiveur.snapshot()).toEqual([])
  })
