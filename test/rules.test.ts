...
describe('règles', () => {
  ...
  it('garde le même identifiant quand la mesure change', () => {
    const one = detect(
      [...healthy(), signal(999, { cost: 50, cacheReadCost: 40 })],
      calibrate(healthy()),
    ).find((f) => f.rule === 'historique-relu');
    const two = detect(
      [...healthy(), signal(999, { cost: 90, cacheReadCost: 80 })],
      calibrate(healthy()),
    ).find((f) => f.rule === 'historique-relu');
  ...
  it('ne signale le socle qu’à partir d’une accumulation', () => {
    ...
    const few = [
      ...healthy(),
      ...Array.from({ length: 3 }, (_, i) => signal(900 + i, { turns: 2 })),
    ];
    ...
    const many = [
      ...
      ...Array.from({ length: 25 }, (_, i) =>
        signal(900 + i, { turns: 2, firstTurnContext: 60_000 }),
      ),
    ];
  ...
  it('désigne une session qui cherche bien plus qu’elle ne construit', () => {
    ...
    const found = detect(signals, calibrate(signals)).find(
      (f) => f.rule === 'exploration-sans-fin',
    );
  ...
  it('lit les tours par prompt à l’envers, et jamais dans l’autre sens', () => {
    ...
    expect(
      detect(beaucoup, calibrate(beaucoup, seuil)).some((f) => f.rule === 'brief-morcele'),
    ).toBe(false);

    const morcele = [...healthy(), signal(999, { turns: 44, userTurns: 4 })]; // 11,0
    const found = detect(morcele, calibrate(morcele, seuil)).find(
      (f) => f.rule === 'brief-morcele',
    );
  ...
  it('rapporte la fenêtre à la limite du modèle, pas à un nombre en dur', () => {
    ...
    const found = detect(signals, calibrate(signals)).filter(
      (f) => f.rule === 'fenetre-proche-limite',
    );
