...
function call(name: string, input: Record<string, unknown>, meta?: Record<string, unknown>, isSidechain = false): TranscriptEvent {
  seq += 1
  return {
    uuid: `u${seq}`,
    parentUuid: null,
...
describe('les plans successifs', () => {
  ...
  it('ne coupe pas un plan posé en deux temps', () => {
    ...
    const events = [call('TaskCreate', { subject: 'A' }), text(), call('TaskCreate', { subject: 'B' })]
...
describe('regroupement des créations dans le flux', () => {
  ...
  it('ne groupe pas une création isolée', () => {
    ...
    const { starts, absorbed, marks } = groupTaskPlans([call('TaskCreate', { subject: 'A' })], never)
  ...
  it('ne fait pas disparaître un jalon qui se déplie', () => {
    ...
    const events = [call('TaskCreate', { subject: 'A' }), call('TaskCreate', { subject: 'B' }), call('TaskCreate', { subject: 'C' })]
  ...
  it('ne groupe pas les mises à jour', () => {
    ...
    const events = [call('TaskUpdate', { taskId: '1', status: 'completed' }), call('TaskUpdate', { taskId: '2', status: 'in_progress' })]
