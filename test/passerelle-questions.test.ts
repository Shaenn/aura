...
import { clavier, courante, ecran, formulaire, presse, repondLibre, reponses } from '../server/passerelle/questions.ts'
...
function rangs(clavier: InlineKeyboardMarkup): string[] {
  return clavier.inline_keyboard.flat().map((b) => ('callback_data' in b ? (b.callback_data ?? '') : ''))
}
...
describe('lՎcran', () => {
  ...
  it('rend une maquette dans son propre bloc, et la borne', () => {
    ...
    const f = formulaire(ID, [{ ...simple, options: [{ label: 'a', description: '', preview: maquette }] }])
