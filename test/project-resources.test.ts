...
import { buildCandidateTree, buildTree, groupSkills, treeCount, type RuleDirNode } from '../src/components/resources/projectResources.ts'
...
describe('buildTree', () => {
  const rules = [res('rules/back/domain.md', 'rules'), res('rules/back/application/contracts.md', 'rules'), res('rules/front.md', 'rules')]
  ...
  it('retire le segment de catégorie quand on le lui demande', () => {
    ...
    expect(tree.map((n) => (n.type === 'dir' ? n.name : n.label)).sort()).toEqual(['back', 'front.md'])
  })
