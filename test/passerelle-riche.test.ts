...
describe('fragments', () => {
  ...
  it('mêle le nu et l’enrichi dans l’ordre de lecture', () => {
    expect(fragments('avant **gras** après')).toEqual(['avant ', { type: 'bold', text: 'gras' }, ' après'])
  })
  ...
  it('relit le libellé d’un lien, dont la cible est écartée', () => {
    ...
    expect(fragments('[`0.livraison.md`](../livraison/0.livraison.md)')).toEqual([{ type: 'marked', text: { type: 'code', text: '0.livraison.md' } }])
  ...
  it('rend un lien, et refuse les protocoles qu’on ne relaie pas', () => {
    expect(fragments('[doc](https://exemple.fr)')).toEqual([{ type: 'url', text: 'doc', url: 'https://exemple.fr' }])
...
describe('enBlocs', () => {
  ...
  it('regroupe les lignes consécutives en un paragraphe', () => {
    expect(enBlocs('une ligne\net sa suite')).toEqual([{ type: 'paragraph', text: ['une ligne et sa suite'] }])
  })
  ...
  it('rend une citation et un séparateur', () => {
    expect(enBlocs('> cité')).toEqual([{ type: 'blockquote', blocks: [{ type: 'paragraph', text: ['cité'] }] }])
    expect(enBlocs('a\n\n---\n\nb')).toEqual([{ type: 'paragraph', text: ['a'] }, { type: 'divider' }, { type: 'paragraph', text: ['b'] }])
  })
