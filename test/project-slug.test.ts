...
describe('encodeProjectPath', () => {
  it('encode les séparateurs et le deux-points du lecteur', () => {
    expect(encodeProjectPath(String.raw`C:\Users\jean.dupont\Documents\devl\aura`)).toBe('c--users-jean-dupont-documents-devl-aura')
  })

  it('encode aussi le souligné, en doublant le tiret du séparateur', () => {
    expect(encodeProjectPath(String.raw`C:\devl\_archives\scanner`)).toBe('c--devl--archives-scanner')
  })

  it('encode le point et l’espace', () => {
    expect(encodeProjectPath(String.raw`C:\devl\mon projet\.claude`)).toBe('c--devl-mon-projet--claude')
  })
