...
describe('treeRows — l’arbre', () => {
  it('emboîte les segments, un par ligne, sans jamais montrer un chemin', () => {
    expect(drawn('index')).toEqual(['src/services/', '  agent/', '    index.ts', '  projects/', '    index.ts'])
  })
...
describe('treeRows — le repli', () => {
  ...
  it('ne replie que la branche visée', () => {
    expect(drawn('index', new Set(['src/services/agent']))).toEqual(['src/services/', '  agent/', '  projects/', '    index.ts'])
  })
