...
describe('ansiToHtml', () => {
  it('rend un passage grisé', () => {
    expect(ansiToHtml(`${ESC}[2mwaiting${ESC}[22m ok`)).toBe(
      '<span class="ansi-dim">waiting</span> ok',
    );
  });
  ...
  it('cumule les attributs et remet à zéro sur 0', () => {
    expect(ansiToHtml(`${ESC}[1m${ESC}[31mgras rouge${ESC}[0mnormal`)).toBe(
      '<span class="ansi-fg-red ansi-bold">gras rouge</span>normal',
    );
  });
  ...
  it('inverse la vidéo en échangeant les deux plans', () => {
    expect(ansiToHtml(`${ESC}[7minv${ESC}[27m`)).toBe(
      '<span class="ansi-fg-black ansi-bg-white">inv</span>',
    );
  });
