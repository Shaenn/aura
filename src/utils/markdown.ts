...
function createRenderer(allowHtml: boolean): MarkdownItInstance {
  ...
  const defaultFence = m.renderer.rules.fence ?? ((tokens, idx, options, _env, self) => self.renderToken(tokens, idx, options))
  ...
  const defaultLinkOpen = m.renderer.rules.link_open ?? ((tokens, idx, options, _env, self) => self.renderToken(tokens, idx, options))
...
const PURIFY: Config = {
  ALLOWED_ATTR: ['href', 'src', 'alt', 'title', 'align', 'width', 'height', 'class', 'target', 'rel', 'colspan', 'rowspan', 'open', 'controls'],
