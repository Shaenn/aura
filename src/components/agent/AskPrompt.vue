<template>
  ...
  <form class="ap" :aria-labelledby="headId" @submit.prevent="submit">
    <header class="ap-head">
      ...
      <q-space />
      <q-btn flat dense round size="sm" icon="close" :aria-label="t('agent.ask.close')" @click="emit('close')">
    ...
    -->
    <q-stepper v-if="multi" v-model="step" flat animated header-nav class="ap-stepper" color="primary" active-color="primary" done-color="primary">
      <q-step
      ...
      >
        <QuestionFields :question="q" :model-value="picks[qi]" @update:model-value="(v) => setPick(qi, v)" />
      </q-step>
    </q-stepper>

    <QuestionFields v-else-if="questions[0]" :question="questions[0]" :model-value="picks[0]" @update:model-value="(v) => setPick(0, v)" />
    ...
    <div class="ap-actions">
      <q-btn v-if="multi && step > 0" flat no-caps icon="chevron_left" :label="t('agent.ask.previous')" :disable="busy" @click="step -= 1" />
      <q-space />
      ...
      <q-btn
        ...
        @click="step += 1"
      />
      <q-btn v-else unelevated no-caps type="submit" color="primary" :label="t('agent.ask.submit')" :disable="busy || !complete">
