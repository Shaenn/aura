  ...
  <section class="pp" :aria-labelledby="headId">
    ...
    <div class="pp-actions">
      <q-btn
        ...
        no-caps
        color="primary"
        :label="t('agent.permission.allow')"
        :disable="busy"
        @click="answer('allow')"
      />
      <q-btn
        flat
        no-caps
        :label="t('agent.permission.always')"
        :disable="busy"
...
import type { PermissionAnswer, PermissionRequest } from 'src/services/agent';
...
const label = computed(
  () => props.request.title || props.request.displayName || props.request.toolName,
);
...
const since = computed(() => {
  ...
  return s < 60
    ? `${s} ${unit}`
    : `${Math.floor(s / 60)} ${t('formats.duration.min')} ${String(s % 60).padStart(2, '0')}`;
});
