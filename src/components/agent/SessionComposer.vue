  ...
  <form class="sc" @submit.prevent="submit">
    ...
    <div class="sc-actions">
      ...
      <q-btn
        ...
        no-caps
        icon="stop_circle"
        :label="t('agent.composer.interrupt')"
        :disable="busy"
...
const placeholder = computed(() =>
  props.ended
    ? t('agent.composer.ended')
    : props.working
      ? t('agent.composer.working')
...
const found = computed(() =>
  menu.value?.kind === 'file'
    ? treeRows(props.files ?? [], menu.value.query, collapsed.value)
    : null,
);
...
const countNote = computed(() =>
  menu.value?.kind === 'file' && props.filesTruncated ? t('agent.composer.files.truncated') : '',
);
...
const activeId = computed(() =>
  menuOpen.value && items.value.length ? `${listId}-${active.value}` : undefined,
);
