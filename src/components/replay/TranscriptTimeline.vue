      ...
      <details class="rp-silent" :open="silentOpen" @toggle="onSilentToggle">
        ...
        <ul class="rp-silent-list">
          <li v-for="g in silentHooks.groups" :key="g.command">
            ...
            <span class="rp-silent-count font-mono"> ×{{ g.count }} · {{ fmtDuration(g.durationMs) }} </span>
          </li>
        </ul>
...
<script setup lang="ts">
  ...
  async function scrollToEvent(uuid: string): Promise<boolean> {
    ...
    const target =
      document.getElementById(`rp-task-${uuid}`) ?? document.getElementById(`rp-turn-${uuid}`) ?? document.getElementById(nodeAnchor(node.uuid))
