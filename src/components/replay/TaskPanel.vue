<template>
  <div class="tk">
    ...
    -->
    <button v-if="past.length" type="button" class="tk-past" :aria-expanded="open" :aria-controls="pastId" @click="open = !open">
      <q-icon :name="open ? 'expand_less' : 'expand_more'" size="16px" aria-hidden="true" />
      <span class="tk-past-name">{{ past.length > 1 ? t('replay.tasks.pastMany') : t('replay.tasks.pastOne') }}</span>
      <q-space />
    ...
    <div v-if="past.length && open" :id="pastId" class="tk-past-body">
      <section v-for="w in past" :key="w.index" class="tk-past-wave">
        ...
        <TaskLines :tasks="w.tasks" :label="t('replay.tasks.planTasks', { n: w.index })" @navigate="emit('navigate', $event)" />
      </section>
    </div>

    <TaskLines v-if="progress.currentWave" :tasks="progress.currentWave.tasks" :label="currentLabel" @navigate="emit('navigate', $event)" />
  </div>
</template>
...
<script setup lang="ts">
   ...
   */
  const currentLabel = computed(() => (props.progress.waves.length > 1 ? t('replay.tasks.currentPlan') : t('replay.tasks.sessionTasks')))
</script>
