  ...
  <div class="auq">
    <div v-for="(q, qi) in questions" :key="qi" class="auq-q">
      <div class="auq-q-head">
        ...
        <span v-if="q.multiSelect" class="auq-multi font-mono">{{ t('agent.ask.multiSelect') }}</span>
      </div>
      <p class="auq-question">{{ q.question }}</p>

      <ul class="auq-options">
        <li v-for="(o, oi) in q.options" :key="oi" class="auq-option" :class="{ 'auq-option--picked': isPicked(qi, o.label) }">
          <q-icon
