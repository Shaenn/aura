    ...
    <template v-else>
      ...
      <header class="cx-head">
        <template v-if="windowTokens > 0">
          <p class="cx-fill font-mono">
            ...
            <span class="cx-fill-limit">{{ t('replay.context.fillLimit', { n: fmtNum(context.limit) }) }}</span>
        ...
        <p v-if="cost !== null" class="cx-cost font-mono">
          ...
          <span class="cx-cost-note"> {{ costPartial ? `${t('replay.context.costFloor')} ` : '' }}{{ t('replay.context.costNote') }} </span>
        </p>
      </header>
      ...
      <ul v-else-if="view === 'category'" class="cx-sections">
        <li v-for="g in groups" :key="g.key">
          <details class="cx-section" :open="openKeys.has(g.key)" @toggle="onSectionToggle(g.key, $event)">
            ...
            <ContextTree v-if="g.asTree" :nodes="g.tree" class="cx-tree" @navigate="emit('navigate', $event)" />
            <ul v-else class="cx-items">
              <ContextRow v-for="(row, i) in g.rows" :key="i" :row="row" @navigate="emit('navigate', $event)" />
            </ul>
          </details>
        </li>
      </ul>
      ...
      <section v-if="lastTurn && unattributed > 0" class="cx-rest">
        ...
        <i18n-t v-if="baseline > 0" keypath="replay.context.baselineNote" tag="p" class="cx-note" scope="global">
          <template #exact>
      ...
      <i18n-t v-if="lastTurn" keypath="replay.context.estimateNote" tag="p" class="cx-note" scope="global">
        <template #estimated>
...
<script setup lang="ts">
  ...
  const phases = computed(() => props.context.compactions.map((_, i) => i).concat(props.context.compactions.length))
  const phase = ref(0)
  ...
  const fillPercent = computed(() => (windowTokens.value > 0 ? Math.round((windowTokens.value / props.context.limit) * 100) : 0))
  ...
  const visible = computed<ContextInjection[]>(() => {
    ...
    if (!turn) return []
    return props.context.injections.filter((i) => i.phase === phase.value && i.turnIndex <= turn.turnIndex && i.tokens > 0)
  })

  const rankedRows = computed(() => [...visible.value].sort((a, b) => b.tokens - a.tokens).map((i) => toRow(props.context, i)))

  const flatRows = computed(() => toFlatRows(props.context, visible.value))

  const groups = computed(() =>
    CATEGORIES.map((key) => {
      const items = visible.value.filter((x) => x.category === key).sort((a, b) => b.tokens - a.tokens)
