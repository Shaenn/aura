...
function detailOf(injection: ContextInjection): ContextDetailLine[] {
  ...
  if (injection.entries?.length) {
    return [...injection.entries].sort((a, b) => b.tokens - a.tokens).map((e) => ({ label: e.label, tokens: e.tokens }))
  }
  return []
}
 ...
 * ailleurs.
 */
export function toTurnGroups(context: SessionContext, phase: number, injections: ContextInjection[]): TurnGroup[] {
  ...
  return turnDeltas(context, phase)
    .map((d): TurnGroup => {
      ...
      return {
        ...
        rows: injs.sort((a, b) => b.tokens - a.tokens).map((i) => ({ ...toRow(context, i), uuid: undefined })),
      }
    })
 ...
 */
export function toFlatRows(context: SessionContext, injections: ContextInjection[]): ContextRowModel[] {
