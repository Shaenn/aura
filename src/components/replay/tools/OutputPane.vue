<template>
  ...
  <section v-if="text || isError" class="op" :class="{ 'op--error': isError }">
    <button type="button" class="op-head" :aria-expanded="open" :aria-controls="bodyId" @click="open = !open">
      <q-icon :name="open ? 'expand_more' : 'chevron_right'" size="16px" aria-hidden="true" />
      <span class="section-label">{{ isError ? t('replay.tools.output.error') : t('replay.tools.output.result') }}</span>
    ...
    <div v-if="open" :id="bodyId" class="op-body">
      <div v-if="persisted" class="op-persisted">
        <p class="op-note">
          Sortie trop volumineuse pour le transcript ({{ persisted.size }}). Claude Code n'en a gardé qu'un aperçu et a écrit le reste sur le disque.
        </p>
        <q-btn
          v-if="canLoad && !full"
          unelevated
      ...
      <div class="op-actions">
        <q-btn
          ...
          size="sm"
          class="op-more"
          :label="clamped ? t('replay.tools.output.seeAll', { n: lineCount }) : t('replay.tools.output.reduce')"
...
<script setup lang="ts">
   ...
   */
  const PERSISTED = /^<persisted-output>\s*\nOutput too large \(([^)]+)\)\. Full output saved to: (.+?)\n/
  ...
  const shown = computed(() => (truncatable.value && clamped.value ? text.value.split('\n').slice(0, CLAMP_LINES).join('\n') : text.value))
