<template>
  <div class="tv">
    ...
    <template v-else>
      <PathList :paths="paths" :root="root" :order="paths.length > 1 ? t('replay.tools.views.glob.order') : ''" />
...
<script setup lang="ts">
  ...
  const paths = computed(() => lines.value.filter((l) => !NOTHING.test(l) && !TRUNCATED.test(l) && !SHOWING.test(l) && !RESULT_ID.test(l)))
