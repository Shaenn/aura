<template>
  <div class="tv">
    ...
    <CodeBlock v-if="stopped?.command" :code="stopped.command" :line-numbers="false" icon="terminal" max-height="200px" />
...
<script setup lang="ts">
  ...
  const params = computed(() =>
    chips([
      ...
      [t(stop.value ? 'replay.tools.chips.background' : 'replay.tools.chips.task'), str(input.value.taskId, str(input.value.task_id))],
      ['statut', str(input.value.status)],
      [t('replay.tools.chips.delay'), num(input.value.delaySeconds) ? t('replay.tools.chips.seconds', { n: num(input.value.delaySeconds) }) : ''],
  ...
  const stopped = computed(() => {
    ...
    if (agent) {
      return {
        what: command ? `Arrêt du sous-agent « ${command} », lancé en arrière-plan.` : t('replay.tools.views.task.stopAgent'),
        command: '',
      }
    }
