    ...
    <ul v-else-if="parsed?.kind === 'arbre'" class="lv-tree">
      <li v-for="(node, i) in parsed.nodes" :key="i" class="lv-node">
        ...
        <span class="lv-branch" :style="{ paddingLeft: `calc(${node.depth} * var(--space-md))` }">
          ...
          <span
            v-if="added(node.detail, node.name)"
            class="lv-detail font-mono"
            :title="node.detail"
          >
          ...
          <ul class="lv-hits">
            <li v-for="(item, ii) in group.items" :key="ii" class="lv-hit">
              ...
              <template v-if="item.label">
                ...
                <span
                  v-if="added(item.detail ?? '', item.label)"
                  class="lv-detail font-mono"
                  :title="item.detail"
                >
              ...
              <span v-if="item.callsAt" class="lv-line">{{
                t('replay.tools.views.lsp.callAt', { at: item.callsAt })
              }}</span>
            </li>
          </ul>
        </li>
 ...
 * personne n'a choisi.
 */
const trivialPosition = computed(
  () => num(input.value.line) === 1 && num(input.value.character) === 1,
);

const position = computed(() =>
  trivialPosition.value ? '' : `:${num(input.value.line)}:${num(input.value.character)}`,
);
...
const kindOf = (kind: string): string =>
  KINDS.has(kind) ? t(`replay.tools.views.lsp.kinds.${kind}`) : kind;
...
const FOUND =
  /^Found (\d+) (reference|symbol|definition|implementation|incoming call|outgoing call)s?(?: in workspace)?(?: across (\d+) files?)?:$/;
...
const NOTHING =
  /^No (?:references|hover information|symbols|definition|implementations|incoming calls|outgoing calls)\b/;
...
const parsed = computed<Parsed>(() => {
  ...
  if (noServer)
    return { kind: 'echec', cause: `Aucun serveur de langage pour les fichiers ${noServer[1]}` };
 ...
 */
const at = (hit: Hit): string =>
  hit.character
    ? `${hit.line}:${hit.character}`
    : `${t('replay.tools.views.lsp.line')} ${hit.line}`;

const emptyLabel = computed(() =>
  EMPTY.has(operation.value)
    ? t(`replay.tools.views.lsp.empty.${operation.value}`)
    : t('replay.tools.views.lsp.noResult'),
);
