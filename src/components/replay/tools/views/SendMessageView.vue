...
<script setup lang="ts">
   ...
   */
  const target = computed(() => str(input.value.to) || str(input.value.recipient) || str(input.value.agentId))
  ...
  const params = computed(() =>
    chips([
      ...
      [t('replay.tools.chips.request'), str(input.value.request_id) || str(asRecord(input.value.message).request_id)],
    ]),
  )
  ...
  const order = computed(() => {
    ...
    if (type === 'shutdown_request') {
      ...
      return {
        icon: 'logout',
        said: reason ? t('replay.tools.views.sendMessage.askStopWhy', { reason }) : t('replay.tools.views.sendMessage.askStop'),
      }
    }
    if (type === 'shutdown_response') {
      ...
      return {
        ...
        said: t(yes ? 'replay.tools.views.sendMessage.approved' : 'replay.tools.views.sendMessage.refused'),
      }
    }
    return null
  })
  ...
  const SHUTDOWN_OK = /^Shutdown approved\. Sent confirmation to ([^.]+)\. Agent (\S+) is now exiting\.$/
