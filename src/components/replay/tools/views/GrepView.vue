  ...
  <div class="tv">
    ...
    <ol v-else class="gv-files">
      <li v-for="group in hits" :key="group.file" class="gv-file">
        ...
        <ul class="gv-lines">
          <li v-for="(m, i) in group.matches" :key="i" class="gv-line" :class="{ 'gv-ctx': m.context }">
...
<script setup lang="ts">
  ...
  const rawOnly = computed(() => props.block.result?.isError === true || raw.value.startsWith('<persisted-output>'))
  ...
  const params = computed(() =>
    chips([
      ...
      [t('replay.tools.chips.caseInsensitive'), bool(input.value['-i']) ? t('replay.tools.chips.yes') : ''],
    ]),
  )
  ...
  function merge(byFile: Map<string, Match[]>): { file: string; matches: Match[] }[] {
    ...
    for (const [key, matches] of byFile) {
      const name = isAbsolute(key) ? (relatives.find((k) => slash(key).endsWith(`/${slash(k)}`)) ?? key) : key
    ...
    return [...out].map(([file, matches]) => ({
      ...
      matches: matches.every((m) => m.line) ? [...matches].sort((a, b) => Number(a.line) - Number(b.line)) : matches,
    }))
  }
