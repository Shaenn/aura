<template>
  <section class="tr" :style="accent ? { borderLeftColor: accent } : undefined">
    <header class="tr-head">
      ...
      <q-badge v-if="block.status" :color="block.status === 'completed' ? 'positive' : 'grey-6'" :label="block.status" rounded />
    </header>

    <MarkdownView v-if="block.text" :source="block.text" />
    <p v-else class="tr-empty">{{ t('replay.report.empty') }}</p>

    <details v-if="block.outputFile || block.note" class="tr-meta" :open="metaOpen" @toggle="onMetaToggle">
      <summary><q-icon name="info" size="13px" aria-hidden="true" /> {{ t('replay.report.details') }}</summary>
