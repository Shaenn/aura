  ...
  <article class="at">
    ...
    >
      ...
      <span v-if="cardContext" class="at-ctx-sum font-mono">
        ...
        <span v-if="cardContext.delta > 0" class="at-ctx-sum-delta"
          >+{{ fmtNum(cardContext.delta) }}</span
        >
      ...
      <span v-if="turn.durationMs" class="at-dur font-mono">{{
    ...
    <div v-if="open" :id="bodyId" class="at-body">
      <template v-for="(ev, ei) in turn.events" :key="ev.uuid">
        ...
        <template v-if="turnAt(ev.uuid) && !plans.marks.has(ev.uuid)">
          <details
            v-if="turnAt(ev.uuid)!.rows.length"
            :id="`rp-turn-${ev.uuid}`"
            class="at-turn at-turn--fold"
          >
            <summary class="at-turn-mark font-mono">
              ...
              <span v-if="turnAt(ev.uuid)!.delta > 0" class="at-turn-delta"
                >+{{ fmtNum(turnAt(ev.uuid)!.delta) }}</span
              >
              <span class="at-turn-silent">{{
                t('replay.turn.injected', turnAt(ev.uuid)!.rows.length)
              }}</span>
            </summary>
            <ul class="at-turn-list">
              <li v-for="(row, i) in turnAt(ev.uuid)!.rows" :key="i" class="at-turn-row">
                <span
                  class="at-turn-pill"
                  :style="{ color: pillColor(row), borderColor: pillColor(row) }"
                  >{{ pill(row) }}</span
                >
        ...
        <p v-if="systemLabel(ev)" class="at-sys" :class="{ 'at-sys--error': ev.level === 'error' }">
          <q-icon
            :name="ev.level === 'error' ? 'error_outline' : 'info'"
            size="13px"
            aria-hidden="true"
          />
          {{ systemLabel(ev) }}
        ...
        <template v-for="(b, bi) in ev.blocks" :key="`${ei}-${bi}`">
          ...
          <TaskMarker
            v-else-if="isTaskMarker(b) && !plans.absorbed.has(blockKey(ei, bi))"
            :block="b"
            :uuid="ev.uuid"
          />

          <ToolCall
            v-else-if="b.kind === 'tool_use' && !plans.absorbed.has(blockKey(ei, bi))"
            :block="b"
          />
 ...
 */
const CAT_ORDER: ContextCategory[] = [
  'memory',
  'skills',
  'files',
  'tools',
...
const plans = computed(() =>
  groupTaskPlans(
    props.turn.events,
    (ev) =>
      Boolean(systemLabel(ev)) ||
      ev.blocks.some((b) => skillDoc(ev, b)) ||
      (turnAt(ev.uuid)?.rows.length ?? 0) > 0,
  ),
);
