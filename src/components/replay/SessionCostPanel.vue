    ...
    <template v-else>
      ...
      <section class="sd-sec">
        ...
        <div class="sd-bar" role="img" :aria-label="breakdownAria">
          <span v-for="p in parts" :key="p.key" class="sd-bar-part" :style="{ width: widthOf(p.value), background: p.color }" />
        </div>
...
<script setup lang="ts">
  ...
  import { getSessionDiagnostic, type Finding, type SessionDiagnostic, type SessionRank, type Severity } from '@/services/diagnostics'
  import { severityIcon, severityColor, severityLabel, worstSeverity } from '@/services/diagnostics/severity'
  ...
  const emit = defineEmits<{
     ...
     */
    summary: [{ totalUsd: number; partial: boolean; worst: Severity | null; findings: number } | null]
  }>()
  ...
  function unitValue(v: number, unit: SessionRank['unit']): string {
    ...
    if (unit === 'tokens') return v >= 1e6 ? `~${(v / 1e6).toFixed(1)} M` : `~${Math.round(v / 1000)} k`
