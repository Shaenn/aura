...
const durationPart = computed(() =>
  props.compaction.durationMs > 0
    ? t('replay.compaction.removedIn', { d: fmtDuration(props.compaction.durationMs) })
    : '',
);
const triggerLabel = computed(() =>
  t(props.compaction.trigger === 'auto' ? 'replay.compaction.auto' : 'replay.compaction.manual'),
);
</script>
