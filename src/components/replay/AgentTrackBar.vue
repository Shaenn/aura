<template>
  ...
  >
    ...
    <q-tab v-for="run in runs" :id="trackTabId(run.agentId)" :key="run.agentId" :name="run.agentId" :ripple="false" class="atb-tab">
      <span class="atb-inner">
        ...
        <q-icon v-if="run.status === 'completed'" name="check" size="13px" class="atb-done" aria-hidden="true" />
        <q-icon v-else-if="run.status === 'failed'" name="error_outline" size="13px" class="atb-failed" aria-hidden="true" />
