        ...
        <ul class="orp-items">
          <li v-for="p in orphans" :key="p.name">
            ...
            >
              ...
              <span class="orp-item-meta font-mono"> {{ fmtDate(p.mtime) }} · {{ fmtBytes(p.size) }} </span>
            </button>
          </li>
        </ul>
      </nav>
      ...
      <article class="orp-view" :aria-label="t('pages.maintenance.orphans.preview')">
        <template v-if="selected">
          <header class="orp-toolbar">
            ...
            <q-space />
            <q-btn flat dense no-caps color="negative" icon="delete" :label="t('common.delete')" @click="confirmDelete = true" />
          </header>
          <div class="orp-content">
            <div v-if="reading" class="orp-loading"><q-spinner size="24px" /> {{ t('common.loading') }}</div>
