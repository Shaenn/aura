...
export function groupSkills(resources: ResourceNode[]): SkillFolder[] {
  ...
  for (const [key, items] of byFolder) {
    const main = items.find((r) => /^SKILL\.md$/i.test(r.name)) ?? (items.length === 1 ? items[0] : undefined) ?? null
    ...
    const refFolders = [...folders.entries()].map(([name, fitems]) => ({ name, items: fitems })).sort((a, b) => a.name.localeCompare(b.name))
    out.push({
      key,
