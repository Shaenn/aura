<template>
  <q-page class="rv">
    ...
    <header class="rv-header">
      ...
      <span class="rv-ro font-mono">
        <q-icon name="lock" size="12px" aria-hidden="true" /> {{ t('common.readOnly') }}
      </span>
    </header>
    ...
    <div class="rv-body">
        ...
        <div v-for="it in items" :key="it.rel" class="rv-item">
          ...
          >
            ...
            <span
              v-else-if="accentFor"
              class="rv-item-dot"
              :style="{ backgroundColor: accentFor(it) }"
              aria-hidden="true"
            />
          ...
          <ul v-if="hasRefs && selected?.rel === it.rel && refFiles.length" class="rv-refs">
            <li v-for="f in refFiles" :key="f.rel">
              <button
                class="rv-ref"
                :class="{ 'rv-ref--active': viewer.rel === f.rel }"
                @click="openFile(f.rel)"
              >
      ...
      <section class="rv-viewer">
        ...
        <template v-else>
          ...
          <div v-if="viewer.loading" class="rv-state surface-card">
...
async function selectItem(it: ResourceItem): Promise<void> {
  ...
  await Promise.all([
    openFile(it.rel),
    refsDir ? loadRefs(refsDir(it.rel), it.rel) : Promise.resolve(),
  ]);
}
...
async function openFile(rel: string): Promise<void> {
  ...
  try {
    ...
    viewer.error =
      e instanceof ClaudeApiError && e.status === 404
        ? 'Fichier introuvable.'
        : e instanceof Error
          ? e.message
          : 'Lecture impossible.';
