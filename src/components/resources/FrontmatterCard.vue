<template>
  <section class="fm surface-card" :aria-labelledby="titleId">
    ...
    <dl v-if="defined.length" class="fm-grid">
      <div v-for="e in defined" :key="e.key" class="fm-row">
        ...
        <dd class="fm-val">
          ...
          <span
            v-else-if="e.spec.kind === 'bool'"
            class="fm-bool"
            :class="e.value === 'true' ? 'fm-bool--on' : 'fm-bool--off'"
          >
          ...
          <ul v-else-if="e.spec.kind === 'globs'" class="fm-globs">
            <li v-for="(g, i) in entryTokens(e)" :key="i">
              <code class="fm-glob">
                <span v-for="(t, ti) in globTokens(g)" :key="ti" :class="{ 'fm-wild': t.wild }">{{
                  t.text
                }}</span>
              </code>
            </li>
    ...
    <div v-if="omitted.length" class="fm-defaults">
      <button
        type="button"
        class="fm-defaults-toggle"
        :aria-expanded="showDefaults"
        @click="showDefaults = !showDefaults"
      >
        <q-icon
          :name="showDefaults ? 'expand_more' : 'chevron_right'"
          size="16px"
          aria-hidden="true"
        />
      ...
      <dl v-if="showDefaults" class="fm-grid fm-grid--muted">
        <div v-for="s in omitted" :key="s.key" class="fm-row">
          <dt class="fm-key font-mono">
            {{ s.key }}
            <q-tooltip anchor="top start" self="bottom start" max-width="320px">{{
              keyInfo(s)
            }}</q-tooltip>
          </dt>
...
const omitted = computed<KeySpec[]>(() =>
  props.keys.filter((s) => !props.entries.some((e) => e.key === s.key)),
);
