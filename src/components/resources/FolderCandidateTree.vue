    ...
    <li v-for="n in nodes" :key="n.rel">
      <div class="fct-row" :class="{ 'fct-row--on': state(n).on }">
        <button
          ...
          :aria-label="
            isOpen(n.rel)
              ? t('common.collapse', { label: n.name })
              : t('common.expand', { label: n.name })
          "
          @click="toggleOpen(n.rel)"
        >
          <q-icon
            :name="isOpen(n.rel) ? 'expand_more' : 'chevron_right'"
            size="14px"
            aria-hidden="true"
          />
        </button>
        ...
        <q-checkbox
          ...
          :aria-label="
            state(n).covered
              ? t('resources.include.coveredAria', { folder: state(n).covered })
              : t('resources.include.toggleAria', { folder: n.rel })
          "
