<template>
  ...
  <div v-if="resources.length || outsideCount || plans.length" class="pd-nav-groups">
    ...
    <div v-if="memories.length" class="pd-group">
      ...
      >
        <q-icon
          :name="isCatOpen('project-memory') ? 'expand_more' : 'chevron_right'"
          size="14px"
          aria-hidden="true"
        />
    ...
    <div v-if="repoDocs.length" class="pd-group">
      <button
        ...
        :aria-label="
          isCatOpen('repo-docs')
            ? t('common.collapse', { label: t('resources.repoDocs') })
            : t('common.expand', { label: t('resources.repoDocs') })
        "
        @click="toggleCat('repo-docs')"
      >
        <q-icon
          :name="isCatOpen('repo-docs') ? 'expand_more' : 'chevron_right'"
          size="14px"
          aria-hidden="true"
        />
    ...
    <div v-for="group in resourceGroups" :key="group.key" class="pd-group">
      <button
        ...
        :aria-label="
          isCatOpen(group.key)
            ? t('common.collapse', { label: group.label })
            : t('common.expand', { label: group.label })
        "
        @click="toggleCat(group.key)"
      >
        <q-icon
          :name="isCatOpen(group.key) ? 'expand_more' : 'chevron_right'"
          size="14px"
          aria-hidden="true"
        />
          ...
          <div v-for="s in skillGroups" :key="s.key" class="pd-skill">
            <div class="pd-skill-head">
              <button
                ...
                :aria-label="
                  isSkillExpanded(s.key)
                    ? t('common.collapse', { label: s.title })
                    : t('common.expand', { label: s.title })
                "
                @click="toggleSkill(s.key)"
              >
                <q-icon
                  :name="isSkillExpanded(s.key) ? 'expand_more' : 'chevron_right'"
                  size="16px"
                  aria-hidden="true"
                />
              </button>
              ...
              >
                ...
                <span v-if="s.refCount" class="pd-skill-badge font-mono">
                  {{ s.refCount }} réf.
                </span>
              </button>
            </div>
            ...
            <div v-if="isSkillExpanded(s.key)" class="pd-skill-body">
              <div v-if="s.refCount" class="pd-skill-refs">
                ...
                <div v-for="f in s.refFolders" :key="f.name" class="pd-ref-folder">
                  ...
                  >
                    <q-icon
                      :name="isRefFolderExpanded(s.key, f.name) ? 'expand_more' : 'chevron_right'"
                      size="14px"
                      aria-hidden="true"
                    />
                  ...
                  <ul
                    v-if="isRefFolderExpanded(s.key, f.name)"
                    class="pd-res-list pd-ref-folder-items"
                  >
        ...
        <ul v-else class="pd-res-list">
          <li v-for="r in group.items" :key="r.rel">
            <button
              ...
              @click="emit('open-resource', r)"
            >
              <span class="pd-res-name font-mono">{{
                group.key === 'agents' ? r.name : r.rel
              }}</span>
              <span v-if="group.key !== 'agents' && r.description" class="pd-res-desc">{{
                r.description
              }}</span>
              <span class="pd-res-size font-mono">{{ fmtBytes(r.size) }}</span>
            </button>
    ...
    <div v-for="f in folders" :key="f.rel" class="pd-group">
      <div class="pd-group-line">
        <button
          ...
          :aria-label="
            isCatOpen(`folder:${f.rel}`)
              ? t('common.collapse', { label: f.rel })
              : t('common.expand', { label: f.rel })
          "
          @click="toggleCat(`folder:${f.rel}`)"
        >
          <q-icon
            :name="isCatOpen(`folder:${f.rel}`) ? 'expand_more' : 'chevron_right'"
            size="14px"
            aria-hidden="true"
          />
    ...
    <div v-if="plans.length" class="pd-group">
      <button
        ...
        :aria-label="
          isCatOpen('plans')
            ? t('common.collapse', { label: t('resources.plans') })
            : t('common.expand', { label: t('resources.plans') })
        "
        @click="toggleCat('plans')"
      >
        <q-icon
          :name="isCatOpen('plans') ? 'expand_more' : 'chevron_right'"
          size="14px"
          aria-hidden="true"
        />
      ...
      <PlanTree
        v-if="isCatOpen('plans')"
        :plans="plans"
        :active-name="activePlanName"
        @open="(p) => emit('open-plan', p)"
      />
    </div>
  </div>
...
const resourcesIn = (category: ResourceCategory): ResourceNode[] =>
  props.resources.filter((r) => r.category === category);
...
const memoryTree = computed<RuleNode[]>(() =>
  buildTree(props.memories, { stripPrefix: false, filesFirst: true }),
);
...
const isRefFolderExpanded = (skill: string, folder: string): boolean =>
  !collapsedRefFolders[refFolderKey(skill, folder)];
