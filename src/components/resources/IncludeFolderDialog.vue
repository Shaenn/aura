<template>
  <q-dialog :model-value="modelValue" @update:model-value="(v) => emit('update:modelValue', v)">
    <q-card class="ifd surface-card">
      ...
      <div v-if="loading" class="ifd-state">
        <q-spinner size="22px" /> {{ t('common.loading') }}
      </div>
      ...
      <footer class="ifd-actions">
        <span class="ifd-tally font-mono">{{
          t('resources.include.selected', selection.length)
        }}</span>
        ...
        <q-btn
          ...
          dense
          color="primary"
          :label="t('resources.include.apply')"
          :disable="!changed"
          @click="apply"
        />
      </footer>
...
import { getFolderCandidates, type FolderCandidate } from 'src/services/projects';
...
function toggle(rel: string, on: boolean): void {
  selection.value = on
    ? [...selection.value.filter((f) => !f.startsWith(`${rel}/`)), rel]
    : selection.value.filter((f) => f !== rel);
}
