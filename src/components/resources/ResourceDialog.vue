...
const parsed = computed(() =>
  ext.value === 'md'
    ? parseDoc(content.value)
    : { present: false, entries: [], body: content.value },
);
const fmKeys = computed<KeySpec[]>(() =>
  props.resource ? (FM_KEYS[props.resource.category] ?? []) : [],
);
