<template>
  ...
  <q-drawer v-model="open" side="right" overlay bordered :width="560" :breakpoint="1024" class="help-drawer" @hide="unpinHelp">
    <section class="hd" aria-labelledby="hd-title">
      <header class="hd-head">
        <q-icon v-if="section" :name="section.icon" size="16px" class="hd-icon" aria-hidden="true" />
        <h2 id="hd-title" class="hd-cmd font-mono">{{ command }}</h2>
        <q-btn flat dense round size="sm" icon="close" :aria-label="t('layout.help.close')" @click="closeHelp" />
      </header>
...
<script setup lang="ts">
  ...
  const manualTo = computed<RouteLocationRaw>(() => (section.value ? { name: 'help', query: { s: section.value.id } } : { name: 'help' }))
</script>
