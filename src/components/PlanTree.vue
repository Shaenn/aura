  ...
  <ul class="pt-years">
    <li v-for="y in years" :key="y.key">
      <button
        ...
        @click="toggle(y.key)"
      >
        <q-icon :name="isOpen(y.key) ? 'expand_more' : 'chevron_right'" size="14px" aria-hidden="true" />
      ...
      <ul v-if="isOpen(y.key)" class="pt-months">
        <li v-for="m in y.months" :key="m.key">
          <button
            ...
            :aria-label="
              isOpen(m.key) ? t('common.collapse', { label: `${m.label} ${y.label}` }) : t('common.expand', { label: `${m.label} ${y.label}` })
            "
            @click="toggle(m.key)"
          >
            <q-icon :name="isOpen(m.key) ? 'expand_more' : 'chevron_right'" size="14px" aria-hidden="true" />
