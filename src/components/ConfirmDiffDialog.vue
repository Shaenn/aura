    ...
    <q-card class="diff-card surface-card surface-card--braced">
      ...
      <div class="diff-body font-mono">
        ...
        <div v-for="(l, i) in lines" :key="i" class="diff-line" :class="`diff-line--${l.op}`">
          <span class="gutter" aria-hidden="true">{{ l.op === 'add' ? '+' : l.op === 'del' ? '−' : ' ' }}</span>
...
<script setup lang="ts">
  ...
  const lines = computed(() => (props.proposal ? diffLines(props.proposal.before, props.proposal.after) : []))
  ...
  async function confirm(): Promise<void> {
    ...
    error.value = ''
    try {
      const res = props.applyFn ? await props.applyFn(props.proposal) : await apply(props.proposal.rel, props.proposal.after, props.proposal.before)
      notifyDone(t('diff.applied'))
      open.value = false
      emit('applied', { rel: res.rel, backupPath: res.backupPath })
    } catch (e) {
      error.value = e instanceof ClaudeApiError && e.status === 409 ? t('diff.conflict') : e instanceof Error ? e.message : t('diff.failed')
    } finally {
