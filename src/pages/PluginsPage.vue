  ...
  <q-page class="pg">
    ...
    <section class="surface-card pg-section" aria-labelledby="pg-mk-title">
      ...
      <q-form ref="mkForm" class="mk-add" @submit.prevent="openAddCommand">
        ...
        <q-btn
          ...
          text-color="dark"
          icon="terminal"
          :label="t('pages.plugins.getCommand')"
        />
      </q-form>
    ...
    <section class="surface-card pg-section" aria-labelledby="pg-plugins-title">
      ...
      <SettingField
        v-for="p in plugins"
        :key="p.id"
        :label="pluginName(p.id)"
        :hint="`v${p.version} · ${p.scope} · ${pluginMarketplace(p.id)}`"
      >
        <SegmentedControl
    ...
    >
      <template v-if="cliMarketplace?.userAdded" #actions>
        <q-btn
          ...
          dense
          color="negative"
          :label="t('pages.plugins.removeFromConfig')"
          @click="removeFromConfig"
        />
      </template>
    </CliCommandDialog>
  </q-page>
