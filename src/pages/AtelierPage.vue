  ...
  <q-page class="at" :class="{ 'at--live': session }" :style-fn="pageStyleFn">
    ...
    <div v-if="!session" class="at-open">
      ...
      <section
        ...
        class="surface-card at-live-open"
        aria-labelledby="at-live-h"
      >
        ...
        <ul class="at-sessions">
          <li v-for="s in liveSessions" :key="s.runId" class="at-live-row">
            <button
              type="button"
              class="at-session"
              :aria-label="t('pages.atelier.live.join', { folder: folderName(s.cwd) })"
              @click="join(s)"
            >
            ...
            <q-btn
              ...
              dense
              icon="close"
              :aria-label="t('pages.atelier.live.stop', { folder: folderName(s.cwd) })"
              @click="stopLive(s.runId)"
            />
          </li>
        </ul>
      </section>
        ...
        <div v-if="cwd.trim()" class="at-resume">
          ...
          <ul v-else-if="resumable.length" class="at-sessions">
            <li v-for="s in resumable" :key="s.id">
              ...
              >
                <span class="at-session-title">{{
                  s.title || s.firstPrompt || t('pages.atelier.untitled')
                }}</span>
                <span class="at-session-meta font-mono">
                  {{ relTime(s.mtime) }}
                  <template v-if="s.metrics">
                    · {{ t('pages.atelier.resumeTurns', s.metrics.turns) }}
                  </template>
                </span>
              </button>
            </li>
        ...
        <div class="at-options">
          <div class="at-option">
            ...
            <SegmentedControl
              v-model="model"
              :options="MODELS"
              :aria-label="t('pages.atelier.modelAria')"
            />
          </div>
          <div class="at-option">
    ...
    <div v-else class="at-live" :class="{ 'at-live--aside': showAside }">
      <div class="at-main">
        <header class="surface-card at-bar">
          ...
          <span
            class="at-dot"
            :class="[`at-dot--${status}`, { 'status-dot--live': status === 'working' }]"
            aria-hidden="true"
          />
          ...
          <span
            v-if="modelBadge"
            class="at-model"
            :class="{ 'at-model--planned': !modelShown?.confirmed }"
            :title="modelTitle"
          >
          ...
          <q-toggle
            ...
            dense
            size="sm"
            :label="t('pages.atelier.followLive')"
            class="at-toggle"
          />
        ...
        -->
        <AgentTrackBar
          v-if="parsed?.subagents.length"
          v-model="track"
          :runs="parsed.subagents"
          :main-count="mainEvents.length"
        />
        ...
        <div class="at-foot">
          ...
          -->
          <button
            v-if="asks.length && !askOpen"
            ref="askReminder"
            type="button"
            class="at-ask-reminder"
            @click="askOpen = true"
          >
            ...
            <span>
              {{
                asks.length > 1
                  ? t('pages.atelier.askMany', { n: asks.length })
                  : t('pages.atelier.askOne')
              }}
            </span>
      ...
      <aside v-if="showAside" class="at-aside" :aria-label="t('pages.atelier.aside')">
        ...
        <section
          v-if="hasWork"
          class="surface-card at-panel at-panel--tasks"
          :class="{ 'at-panel--folded': !workOpen }"
        >
          <div class="at-work-head">
            <q-tabs v-model="workTab" dense no-caps align="left" class="at-tabs">
              ...
              <q-tab v-if="shells.length" name="shells">
                ...
                <span v-if="runningShells" class="at-panel-count font-mono">{{
                  runningShells
                }}</span>
              </q-tab>
            </q-tabs>
            ...
            <q-tab-panel name="shells" class="at-tabpanel">
              <div class="at-panel-scroll">
                <ShellPanel
                  ...
                  :shells="shells"
                  @navigate="(u: string) => void goToShell(u)"
                />
              </div>
            </q-tab-panel>
          </q-tab-panels>
...
const MODELS = computed(() => [
  {
    ...
    value: '',
    tooltip: settingsModel.value
      ? t('pages.atelier.models.settingsTip', { model: settingsModel.value })
      : t('pages.atelier.models.settingsNone'),
  },
...
const settingsModeOffered = computed(() =>
  MODE_LABELS.value.some((m) => m.value === settingsMode.value),
);
...
const MODES = computed(() =>
  MODE_LABELS.value.map((m) => ({
    ...
    tooltip:
      m.value === settingsMode.value
        ? t('pages.atelier.modes.fromSettings', { tip: m.tooltip })
        : m.tooltip,
  })),
);
...
const modelTitle = computed(() => {
  ...
  return session.value?.model
    ? t('pages.atelier.modelChosen', { id: shown.id })
    : t('pages.atelier.modelInherited', { id: shown.id });
});
...
const mainEvents = computed(() =>
  stitch(
    parsed.value ? eventsOfTrack(parsed.value.events, MAIN_TRACK) : [],
    liveMain.value,
    seamTurns.value,
  ),
);
...
const shownEvents = computed(() =>
  track.value === MAIN_TRACK
    ? mainEvents.value
    : eventsOfTrack(parsed.value?.events ?? [], track.value),
);
...
const uuidByToolUse = computed(() => {
  ...
  for (const ev of parsed.value?.events ?? [])
    for (const block of ev.blocks)
      if (block.kind === 'tool_use' && block.id) out.set(block.id, ev.uuid);
  return out;
});
