<template>
  <q-page class="pd">
    ...
    <header class="pd-header">
      <q-icon v-if="detail?.path" name="folder" size="14px" class="pd-path-icon" aria-hidden="true" />
      ...
      <q-btn flat dense no-caps icon="refresh" :label="t('common.refresh')" class="pd-refresh" :disable="loading" @click="refresh" />
    </header>
        ...
        <article class="surface-card pd-viewer" :aria-label="t('pages.project.viewerAria')">
          <template v-if="hasSelection">
            <header class="pd-viewer-head">
              ...
              />
              <span v-else class="pd-ro font-mono"><q-icon name="lock" size="12px" /> {{ t('common.readOnly') }}</span>
            </header>
            <div class="pd-viewer-body">
              <div v-if="viewer.loading" class="pd-viewer-loading"><q-spinner size="24px" /> {{ t('common.loading') }}</div>
      ...
      <q-dialog v-model="confirmPlanDelete">
        <q-card class="pd-del surface-card">
          ...
          <div class="pd-del-actions">
            <q-btn flat no-caps dense :label="t('common.cancel')" @click="confirmPlanDelete = false" />
            <q-btn unelevated no-caps dense color="negative" :label="t('common.delete')" :loading="deletingPlan" @click="doDeletePlan" />
          </div>
        </q-card>
      </q-dialog>

      <!-- Hooks -->
      <section v-if="detail.hooks.length" class="surface-card pd-section" aria-labelledby="pd-hooks-title">
      ...
      <section class="surface-card pd-section" aria-labelledby="pd-tr-title">
        ...
        >
          ...
          <template #body-cell-session="cell">
            <q-td :props="cell">
              <router-link :to="{ name: 'session', params: { slug, id: cell.row.id } }" class="pd-session-link pd-prompt">{{
                cell.value || '—'
              }}</router-link>
              <q-icon v-if="cell.row.titleSource === 'custom'" name="label" size="13px" class="pd-title-badge">
                <q-tooltip>{{ t('pages.project.customTitle') }}</q-tooltip>
              </q-icon>
              <q-icon v-if="cell.row.hasSidechain" name="account_tree" size="13px" class="pd-sub-badge">
    ...
    <IncludeFolderDialog v-model="folderPicker" :slug="slug" :included="settings.foldersOf(slug)" @apply="applyFolders" />
  </q-page>
</template>
...
<script setup lang="ts">
  ...
  import { CATEGORY_META, FM_KEYS, renderBody, treeCount } from '@/components/resources/projectResources'
  ...
  const navCount = computed(() => (detail.value ? treeCount({ ...detail.value, plans: projectPlans.value }) : 0))

  const winPath = computed(() => (detail.value?.path ? detail.value.path.replace(/\//g, '\\') : ''))
  const claudeSize = computed(() => (detail.value ? detail.value.resources.reduce((sum, r) => sum + r.size, 0) : 0))
  ...
  const visibleColumns = computed(() => columns.value.map((c) => c.name).filter((n) => !($q.screen.lt.md && (n === 'gitBranch' || n === 'size'))))
  ...
  const hasSelection = computed(() => (viewer.source === 'plan' ? Boolean(viewer.planName) : Boolean(viewer.rel)))
  ...
  const viewerIcon = computed(() => (viewer.category === 'repo' ? 'article' : VIEWER_ICON[viewer.source]))
  ...
  async function openFile(source: 'resource' | 'memory' | 'included', r: ResourceNode): Promise<void> {
    ...
    rememberSelection()
    try {
      const read = source === 'memory' ? readMemory : source === 'included' ? readIncludedFile : readResource
  ...
  const parsed = computed(() => (viewer.ext === 'md' ? parseDoc(viewer.content) : { present: false, entries: [], body: viewer.content }))
  ...
  async function refresh(): Promise<void> {
    ...
    try {
      ...
      setBreadcrumbs([{ label: t('nav.projects'), to: { name: 'projects' } }, { label: detail.value.name }])
