<template>
  <q-page class="hk">
    ...
    <section
      v-if="pluginHooks.length"
      class="surface-card hk-plugins"
      :aria-label="t('pages.hooks.pluginHooks')"
    >
    ...
    <section
      v-for="ev in eventsWithHooks"
      :key="ev"
      class="surface-card hk-event"
      :aria-label="t('pages.hooks.eventAria', { event: ev })"
    >
      <header class="hk-event-head">
      ...
      <div v-for="(g, gi) in groupsOf(ev)" :key="gi" class="hk-group">
        ...
        <q-btn
          ...
          color="primary"
          icon="add"
          :label="t('pages.hooks.addAction')"
          @click="addAction(ev, gi)"
        />
      </div>
    ...
    <div class="hk-add surface-card">
      <div class="hk-add-row">
        ...
        <q-select
          ...
          options-dense
          class="hk-newev"
          :aria-label="t('pages.hooks.event')"
        />
        <q-btn
          unelevated
          no-caps
          ...
          text-color="dark"
          icon="add"
          :label="t('pages.hooks.add')"
          @click="addGroup(newEvent)"
        />
      </div>
...
const eventsWithHooks = computed(() =>
  Object.keys(hooks.value).filter(
    (e) => Array.isArray(hooks.value[e]) && (hooks.value[e] as unknown[]).length,
  ),
);
const groupsOf = (ev: string): Record<string, unknown>[] =>
  (getAt(parsed.value, ['hooks', ev]) as Record<string, unknown>[] | undefined) ?? [];
...
const pluginEnabled = (id: string): boolean =>
  getAt(parsed.value, ['enabledPlugins', id]) !== false;
...
const hooksEnabled = computed({
  ...
  set: (v: boolean): void =>
    mutate((o) => (v ? deleteAt(o, ['disableAllHooks']) : setAt(o, ['disableAllHooks'], true))),
});
