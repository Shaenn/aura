  ...
  <q-page class="mt">
    ...
    <header class="mt-header">
      ...
      <q-btn
        ...
        dense
        no-caps
        :label="t('pages.maintenance.reload')"
        class="mt-refresh"
        :disable="loading"
        @click="reloadAll"
      />
    ...
    <section class="surface-card mt-section" aria-labelledby="mt-storage-title">
      ...
      <ul class="mt-bars">
        <li v-for="a in allAreas" :key="a.key" class="mt-bar">
          ...
          />
          <span v-else class="mt-bar-protected font-mono">{{
            a.purgeable ? '' : t('pages.maintenance.protected')
          }}</span>
        </li>
      </ul>
    </section>
    ...
    <section class="surface-card mt-section" aria-labelledby="mt-proc-title">
      ...
      <ul v-else class="mt-procs">
        <li
          ...
          :key="row.p.pid"
          class="mt-proc"
          :style="{ paddingLeft: `calc(${row.depth} * var(--space-lg))` }"
        >
          <span class="status-dot" :class="dotClass(row.p)" aria-hidden="true" />
          <span class="mt-proc-kind">{{
            t(`pages.maintenance.processes.kind.${row.p.kind}`)
          }}</span>
          ...
          <span class="mt-proc-age font-mono">{{
            row.p.startedAt ? relTime(row.p.startedAt) : ''
          }}</span>
...
function confirmKill(p: ClaudeProcess): void {
  ...
  $q.dialog({
    ...
    message: n
      ? t('pages.maintenance.processes.confirm.tree', { pid: p.pid, n })
      : t('pages.maintenance.processes.confirm.single', { pid: p.pid }),
...
function purge(area: string, label: string): void {
  ...
  $q.dialog({
    ...
    message: all
      ? t('pages.maintenance.confirm.transcriptsMessage')
      : t('pages.maintenance.confirm.areaMessage', { area: label }),
    cancel: { flat: true, noCaps: true, label: t('common.cancel') },
    ok: {
      color: 'negative',
      noCaps: true,
