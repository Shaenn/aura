    ...
    <template v-else-if="data">
      ...
      <header class="surface-card rp-summary">
        <div class="rp-summary-main">
          <h1 class="rp-title">
            ...
            <span v-if="data.titleSource === 'custom'" class="rp-title-badge font-mono">{{ t('pages.replay.named') }}</span>
          </h1>
          <div class="rp-meta font-mono">
            <span v-if="data.gitBranch"><q-icon name="account_tree" size="13px" /> {{ data.gitBranch }}</span>
            <span v-if="data.stats.models.length"><q-icon name="memory" size="13px" /> {{ data.stats.models.join(', ') }}</span>
            <span><q-icon name="schedule" size="13px" /> {{ fmtDate(data.stats.startedAt) }}</span>
            <span v-if="data.hasSidechain" class="rp-meta-sub"><q-icon name="account_tree" size="13px" /> {{ t('pages.replay.subagents') }}</span>
          </div>
        </div>
      ...
      <div class="rp-cols">
        <div class="rp-main">
          ...
          <div class="rp-head">
            ...
            <div class="rp-controls">
              ...
              <q-space />
              <q-btn flat dense no-caps size="sm" icon="unfold_more" :label="t('pages.replay.expandAll')" @click="expandAll" />
              <q-btn flat dense no-caps size="sm" icon="unfold_less" :label="t('pages.replay.collapseAll')" @click="collapseAll" />
            </div>
        ...
        <aside class="rp-aside" :aria-label="t('pages.replay.aside')">
          ...
          -->
          <AsideSection v-if="tasks.tasks.length" :title="t('pages.replay.tasks')" :open="openPanel === 'tasks'" @toggle="togglePanel('tasks')">
            <template #summary>
              <span v-if="tasks.currentWave"> {{ tasks.currentWave.done }} / {{ tasks.currentWave.tasks.length }} </span>
            </template>
            <TaskPanel :progress="tasks" @navigate="(u: string) => void goToTurn(u)" />
          </AsideSection>

          <AsideSection :title="t('pages.replay.context')" :open="openPanel === 'context'" @toggle="togglePanel('context')">
            ...
            <ContextPanel :context="data.context" @navigate="(u: string) => void goToTurn(u)" @summary="fill = $event" />
          </AsideSection>

          <!-- Le panneau ne parle plus seulement d'argent : il situe aussi ce
               que la session a fait — chercher, relire. D'où le titre. -->
          <AsideSection :title="t('pages.replay.diagnostic')" :open="openPanel === 'cost'" pad @toggle="togglePanel('cost')">
            <!--
...
<script setup lang="ts">
  ...
  const scopeLabel = computed(() => {
    ...
    if (!run) {
      return data.value?.subagents.length ? t('pages.replay.scope.main') : t('pages.replay.scope.all')
    }
