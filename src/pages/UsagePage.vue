  ...
  <q-page class="us">
    ...
    <q-banner dense class="us-caveat">
      ...
      <span v-if="report?.unpricedModels.length" class="us-caveat-unpriced">
        {{
          t(
            'pages.usage.unpriced',
            { n: report.unpricedModels.length, list: report.unpricedModels.join(', ') },
      ...
      <section class="surface-card us-section" aria-labelledby="us-agents-title">
        ...
        <ul v-else class="us-bars">
          <li v-for="a in report.byAgent" :key="a.agentType" class="us-bar">
            ...
            <div class="us-bar-track">
              <div
                class="us-bar-fill us-bar-fill--agent"
                :style="{ width: barWidth(a.cost, report.byAgent[0]?.cost) }"
              />
            </div>
          </li>
        </ul>
...
function colorOf(model: string): string {
  ...
  return i >= 0 && i < tokens.value.series.length
    ? seriesColor(tokens.value, i)
    : tokens.value.muted;
}
...
const chartModels = computed(() =>
  colorKeys.value
    .slice(0, tokens.value.series.length)
    .filter((m) => report.value?.byModel.some((x) => x.model === m)),
);
