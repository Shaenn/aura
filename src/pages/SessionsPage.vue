<template>
  <q-page class="sp" :style-fn="pageStyleFn">
    ...
    <div v-if="loaded && !sessions.length" class="sp-empty">
      ...
      <q-btn flat no-caps :label="t('common.refresh')" :disable="loadingList" @click="loadSessions" />
    </div>
    ...
    <div v-else class="sp-split" :class="{ 'sp-split--aside': showContext }">
      ...
      <nav class="sp-list surface-card" :aria-label="t('pages.sessions.title')">
        ...
        <ul class="sp-groups">
          <li v-for="g in groups" :key="g.key" class="sp-group">
            <button type="button" class="sp-group-head" :aria-expanded="!collapsed.has(g.key)" @click="toggleGroup(g.key)">
              <q-icon :name="collapsed.has(g.key) ? 'chevron_right' : 'expand_more'" size="16px" class="sp-group-caret" aria-hidden="true" />
              ...
              -->
              <span v-if="g.live" class="status-dot status-dot--live status-dot--pulse sp-group-dot" aria-hidden="true" />
              <span class="sp-count font-mono">
                {{ g.sessions.length }}
                <span class="sr-only">
                  {{ t('pages.sessions.groupCount', g.sessions.length) }}{{ g.live ? t('pages.sessions.groupLive', { n: g.live }) : '' }}
                </span>
              </span>
            </button>
      ...
      <section ref="scrollEl" class="sp-stream" :aria-label="t('pages.sessions.stream')" @scroll="onScroll">
        ...
        <template v-else>
          ...
          <div class="sp-stream-top">
            <header class="sp-stream-head">
              <div class="sp-stream-title">
                ...
                <span class="sp-badge font-mono" :class="`sp-badge--${streamState}`">{{ badgeLabel }}</span>
              ...
              />
              <q-toggle v-model="followLive" dense size="sm" :label="t('pages.sessions.followLive')" class="sp-toggle" />
          ...
          -->
            <AgentTrackBar v-if="data?.subagents.length" v-model="track" :runs="data.subagents" :main-count="mainEventCount" />
            ...
            <div v-if="isWaiting" class="sp-perm" role="status">
              <div class="sp-perm-top">
                ...
                <span v-if="pendingTool" class="sp-perm-tool font-mono">{{ pendingTool.name }}</span>
              </div>
              <pre v-if="pendingCmd" class="sp-perm-cmd font-mono"><code>{{ pendingCmd }}</code></pre>
          ...
          <LiveTurnLine :active="turnRunning" :since="lastEventAt" :tool="unresolvedTool?.name" class="sp-live" />
        </template>
      </section>
        ...
        <section class="surface-card sp-panel" :aria-label="t('pages.sessions.panelAria')">
          ...
          <q-tab-panels v-model="asideTab" keep-alive class="sp-tabpanels">
            <q-tab-panel name="context">
              ...
              <ContextPanel :context="data.context" live @navigate="(u: string) => void goToTurn(u)" />
            </q-tab-panel>
    ...
    <ResourceDialog :slug="selected?.slug ?? ''" :resource="openResource" :source="openSource" @close="openResource = null" />
...
<script setup lang="ts">
  ...
  import { eventsOfTrack, trackOfEvent, trackTabId, MAIN_TRACK, type TrackId } from '@/composables/useAgentTracks'
  import { useNotify } from '@/composables/useNotify'
  import { readFile as readClaudeFile, propose as proposeWrite, type Proposal } from '@/services/claude'
  import { onClaudeChange } from '@/services/events'
  import { readTranscript, type ParsedTranscript, type Block, type ResourceNode } from '@/services/projects'
  ...
  const selected = computed(() => sessions.value.find((s) => s.sessionId === selectedId.value) ?? null)
  ...
  const track = computed<TrackId>({
    get: () => (data.value?.subagents.some((r) => r.agentId === wantedTrack.value) ? wantedTrack.value : MAIN_TRACK),
    set: (v) => {
      wantedTrack.value = v
    },
  })
  ...
  const currentRun = computed(() => data.value?.subagents.find((r) => r.agentId === track.value) ?? null)
   ...
   */
  const showContext = computed(() => Boolean(contextColumn.value && selected.value?.slug && data.value))
  ...
  const groups = computed<SessionGroup[]>(() => {
    ...
    for (const s of sessions.value) {
      ...
      if (q && !name.toLowerCase().includes(q) && !winPath(s.cwd).toLowerCase().includes(q) && !sessionLabel(s).toLowerCase().includes(q)) {
        continue
      }
  ...
  async function readTranscriptOnce(showLoader: boolean): Promise<void> {
    ...
    try {
      ...
      if (showLoader || !data.value) error.value = e instanceof Error ? e.message : t('pages.sessions.error')
    } finally {
      firstLoad.value = false
    }
  }
