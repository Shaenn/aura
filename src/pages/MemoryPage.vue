    ...
    <div class="mem-body">
      ...
      <aside class="mem-list surface-card">
        ...
        <div class="mem-group">
          ...
          <button
            class="mem-item"
            :class="{ 'mem-item--active': selected === 'CLAUDE.md' }"
            @click="selectPlain('CLAUDE.md')"
          >
            <span class="mem-item-name"
              ><q-icon name="public" size="14px" aria-hidden="true" /> CLAUDE.md</span
            >
        ...
        <div v-for="p in projects" :key="p.slug" class="mem-group">
          <div class="mem-group-head">
            <button
              ...
              :aria-label="
                isOpen(p.slug)
                  ? t('common.collapse', { label: prettySlug(p.slug) })
                  : t('common.expand', { label: prettySlug(p.slug) })
              "
              @click="toggle(p.slug)"
            >
              <q-icon
                :name="isOpen(p.slug) ? 'expand_more' : 'chevron_right'"
                size="14px"
                aria-hidden="true"
              />
          ...
          <div v-if="isOpen(p.slug)" class="mem-children">
            ...
            >
              <span class="mem-item-name"
                ><q-icon name="toc" size="14px" aria-hidden="true" /> MEMORY.md</span
              >
            ...
            <button
              ...
              class="mem-item"
              :class="[
                { 'mem-item--active': selected === it.rel },
                it.type ? `mem-item--${it.type}` : '',
              ]"
            ...
            <div v-if="mode === 'structured'" class="mem-fields">
              <div class="mem-field">
                <label class="mem-field-label"
                  >{{ t('pages.memory.fields.name') }}
                  <code class="mem-field-key font-mono">name</code></label
                >
              ...
              <div class="mem-field">
                <label class="mem-field-label"
                  >{{ t('pages.memory.fields.description') }}
                  <code class="mem-field-key font-mono">description</code></label
                >
                <textarea
                  :value="description"
                  rows="2"
              ...
              <div class="mem-field">
                <label class="mem-field-label"
                  >{{ t('pages.memory.fields.type') }}
                  <code class="mem-field-key font-mono">metadata.type</code></label
                >
            ...
            <div class="mem-bodyfield">
              <div class="mem-bodyfield-head">
                <label class="mem-field-label" :for="bodyId">
                  {{
                    mode === 'structured'
                      ? t('pages.memory.fields.content')
                      : t('pages.memory.fields.file')
                  }}
                ...
                <SegmentedControl
                  v-model="view"
                  :options="viewOptions"
                  :aria-label="t('pages.memory.viewAria')"
                />
              </div>
              <textarea
                v-if="view === 'edit'"
      ...
      <q-card class="cfm-card surface-card">
        ...
        <div class="cfm-actions">
          ...
          <q-btn
            ...
            dense
            color="negative"
            :label="t('pages.memory.discardConfirm')"
            @click="runPending"
          />
        </div>
      </q-card>
...
const typeInfo = computed<Record<string, string>>(() =>
  Object.fromEntries(TYPES.map((k) => [k, t(`pages.memory.types.${k}`)])),
);
...
const nameTaken = computed(() => {
  ...
  return projects.value.some(
    (p) => p.indexRel?.toLowerCase() === rel || p.items.some((i) => i.rel.toLowerCase() === rel),
  );
});
...
watch([selected, projects], () => {
  ...
  const owner = projects.value.find(
    (p) => rel === p.indexRel || p.items.some((i) => i.rel === rel),
  );
...
const crumb = computed(() => {
  ...
  const p = projects.value.find((x) =>
    dir ? x.memRel === dir : rel === x.indexRel || rel.startsWith(`${x.memRel}/`),
  );
