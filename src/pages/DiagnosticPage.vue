  ...
  <q-page class="dg">
    ...
    <header class="dg-header">
      ...
      <SegmentedControl
        ...
        :aria-label="t('pages.usage.rangeAria')"
        class="dg-range"
      />
    ...
    <template v-else>
      ...
      <section class="surface-card dg-section" aria-labelledby="dg-actions-title">
        <header class="dg-section-head">
          ...
          <span class="dg-count font-mono">{{
            report ? `${report.summary.top.length}` : '—'
          }}</span>
        </header>
        ...
        <template v-else-if="pace">
          <div class="dg-pace">
            ...
            <article class="dg-pace-card">
              <p class="section-label dg-tile-label">
                <HelpTip
                  :what="t('pages.diagnostic.concurrencyWhat')"
                  :reading="t('pages.diagnostic.concurrencyReading')"
                >
          ...
          >
            <template #header>
              ...
              <q-item-section>
                ...
                <span class="dg-reco-meta font-mono">
                  ...
                  <template v-if="!r.calibrated">{{
                    t('pages.diagnostic.uncalibratedTag')
                  }}</template>
                </span>
              </q-item-section>
              <q-item-section side>
            ...
            <div class="dg-reco-body">
              ...
              <p class="dg-reco-basis">
                <q-icon
                  :name="r.impact.kind === 'measured' ? 'straighten' : 'blur_on'"
                  size="14px"
                  aria-hidden="true"
                />
                {{
                  r.impact.kind === 'measured'
                    ? t('pages.diagnostic.measured')
                    : t('pages.diagnostic.estimated')
                }}
                —
                {{ r.impact.basis }}
              </p>
              ...
              <template v-if="r.targets.length && r.scopeIsSession">
                ...
                <ul class="dg-targets">
                  <li v-for="t in r.targets" :key="t.id">
                    <router-link
                      class="dg-target"
                      :to="{ name: 'session', params: { slug: t.project, id: t.id } }"
                    >
                      <span
                        class="dg-target-dot"
      ...
      <section class="surface-card dg-section" aria-labelledby="dg-thresholds-title">
        <header class="dg-section-head">
          ...
          <span class="dg-count font-mono">
            {{
              report
                ? report.thresholds.reliable
                  ? t('pages.diagnostic.thresholdsCalibrated')
                  : t('pages.diagnostic.thresholdsPartial')
                : '—'
            }}
          </span>
        </header>
                ...
                <tr v-for="c in calibrations" :key="c.metric">
                  ...
                  <td class="dg-bound">
                    <HelpTip :what="boundWhat(c)" :reading="c.guardBasis">
                      ...
                      <span v-else-if="c.bound === 'guard'">{{
                        t('pages.diagnostic.boundGuard')
                      }}</span>
      ...
      <section
        v-if="report?.summary.caveats.length"
        class="surface-card dg-section dg-caveats"
        aria-labelledby="dg-caveats-title"
      >
        <header class="dg-section-head">
...
function workReadingFor(key: 'explorationRatio' | 'turnsPerPrompt'): string {
  ...
  if (key === 'explorationRatio') {
    return more
      ? t('pages.diagnostic.readings.exploreMore')
      : t('pages.diagnostic.readings.exploreLess');
  }
...
function boundWhat(c: Calibration): string {
  ...
  const which = t(
    c.direction === 'high' ? 'pages.diagnostic.bound.whichHigh' : 'pages.diagnostic.bound.whichLow',
  );
  ...
  const weight =
    c.hits === 0
      ...
      : t('pages.diagnostic.bound.weight', {
          ...
          cost:
            c.hitsCost !== null
              ? t('pages.diagnostic.bound.weightCost', { cost: money(c.hitsCost) })
              : '',
        });
    ...
    const silenced = s
      ? t(
          'pages.diagnostic.bound.silenced',
          {
            ...
            cost:
              s.cost !== null
                ? t('pages.diagnostic.bound.silencedCost', { cost: money(s.cost) })
                : '',
          },
          s.sessions,
        )
  ...
  const verdict = t(
    p === guard ? 'pages.diagnostic.bound.verdictSame' : 'pages.diagnostic.bound.verdictPassed',
  );
...
function readThreshold(c: Calibration): string {
  ...
  const side = t(
    c.direction === 'high' ? 'pages.diagnostic.read.sideHigh' : 'pages.diagnostic.read.sideLow',
  );
  ...
  if (!c.calibrated) {
    return (
      t('pages.diagnostic.read.uncal', { n: c.sampleSize, value }, c.sampleSize) + sense + dist
    );
  }
  if (c.bound === 'percentile') {
    return (
      t('pages.diagnostic.read.percentile', { rank, n: c.sampleSize, side, value, hits: c.hits }) +
      sense +
      dist
    );
  }
  ...
  const silenced = c.silenced
    ? t('pages.diagnostic.read.guardSilenced', {
        ...
        cost:
          c.silenced.cost !== null
            ? t('pages.diagnostic.read.guardSilencedCost', { cost: money(c.silenced.cost) })
            : '',
      })
    : '';
  return (
